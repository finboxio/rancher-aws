resource "aws_s3_bucket" "rancher-staging-maintenance-bucket" {
  bucket = "maintenance.staging.finbox.io"
  force_destroy = true
  acl = "public-read"
  ...
  policy = <<POLICY
{
  ...
  "Statement":[{
    ...
    "Principal": "*",
    "Action": "s3:GetObject",
    "Resource": "arn:aws:s3:::maintenance.staging.finbox.io/*"
  }]
}
POLICY
}

resource "aws_cloudfront_distribution" "rancher-staging-maintenance-distribution" {
  origin {
    domain_name = "${aws_s3_bucket.rancher-staging-maintenance-bucket.bucket}.s3.amazonaws.com"
    origin_id   = "finboxio-staging-maintenance-origin"
  }

  enabled = true
  comment = "Maintenance page for staging.finbox.io"
  default_root_object = "no.file"

  aliases = [ "*.staging.finbox.io" ]

  default_cache_behavior {
    allowed_methods = [ "GET", "HEAD", "OPTIONS" ]
    cached_methods = [ "GET", "HEAD" ]
    target_origin_id = "finboxio-staging-maintenance-origin"
    compress = true
...
resource "aws_cloudfront_distribution" "rancher-staging-404-distribution" {
  origin {
    domain_name = "${aws_s3_bucket.rancher-staging-maintenance-bucket.bucket}.s3.amazonaws.com"
    origin_id   = "finboxio-staging-404-origin"
  }

  enabled = true
  comment = "404 page for staging.finbox.io"
  default_root_object = "no.file"

  aliases = [ "null.staging.finbox.io" ]

  default_cache_behavior {
    allowed_methods = [ "GET", "HEAD", "OPTIONS" ]
    cached_methods = [ "GET", "HEAD" ]
    target_origin_id = "finboxio-staging-404-origin"
    compress = true
...
resource "aws_route53_record" "rancher-staging-maintenance-dns" {
  zone_id = "${var.zone_id}"
  name = "maintenance.staging.finbox.io"
  type = "A"

  alias {
    name = "${aws_cloudfront_distribution.rancher-staging-maintenance-distribution.domain_name}"
    zone_id = "${aws_cloudfront_distribution.rancher-staging-maintenance-distribution.hosted_zone_id}"
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "rancher-staging-404-dns" {
  zone_id = "${var.zone_id}"
  name = "null.staging.finbox.io"
  type = "A"

  alias {
    name = "${aws_cloudfront_distribution.rancher-staging-404-distribution.domain_name}"
    zone_id = "${aws_cloudfront_distribution.rancher-staging-404-distribution.hosted_zone_id}"
    evaluate_target_health = false
  }
}

resource "aws_route53_record" "rancher-staging-failover-dns" {
  zone_id = "${var.zone_id}"
  name    = "staging.finbox.io"
  type    = "A"

  set_identifier = "secondary-finboxio-staging-dns"
...
resource "aws_route53_record" "rancher-staging-failover-wildcard-dns" {
  zone_id = "${var.zone_id}"
  name    = "*.staging.finbox.io"
  type    = "A"

  set_identifier = "secondary-finboxio-staging-wildcard-dns"
  failover_routing_policy {
    type = "SECONDARY"
  }
...
resource "aws_s3_bucket_object" "013bcd6a3b2bbce663fab28c82d7f9f7c892cd9d" {
  bucket = "${aws_s3_bucket.rancher-staging-maintenance-bucket.bucket}"
