resource "aws_iam_role" "rancher-ec2-iam-role" {
  name               = "rancher-${lower(var.deployment_id)}-${lower(var.environment)}-${lower(var.group)}-ec2-iam-role"
  path               = "/"
...
resource "aws_iam_instance_profile" "rancher-ec2-iam-profile" {
  name  = "rancher-${lower(var.deployment_id)}-${lower(var.environment)}-${lower(var.group)}-ec2-iam-profile"
  path  = "/"
...
resource "aws_iam_policy" "rancher-ec2-iam-policy" {
  name   = "rancher-${lower(var.deployment_id)}-${lower(var.environment)}-${lower(var.group)}-ec2-iam-policy"
  path   = "/"
...
resource "aws_iam_policy_attachment" "rancher-ec2-iam-attachment" {
  name       = "rancher-${lower(var.deployment_id)}-${lower(var.environment)}-${lower(var.group)}-ec2-iam-attachment"
  roles      = [ "${aws_iam_role.rancher-ec2-iam-role.name}" ]
